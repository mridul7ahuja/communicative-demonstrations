...
"""
    Visualizes the belief states of the trajectory by plotting the pmf over the possible colour rewards at each step. 
    Inputs: 
        jointTraj: list of (objectState, beliefState) tuples over the trajectory
        orderOfTrapColours: ordered list of possible trap colours to set the x-axis labels corresponding to each colour reward
        figSize = tuple for the figure size of the entire plot
"""
    
def beliefStateInTrajectoryGraph(jointTraj, orderOfTrapColours, figSize = (10,20)):
    rows = len(jointTraj)
    fig, ax = plt.subplots(rows, 1, sharex = 'col', sharey = 'col', figsize= figSize)
    for i in range(len(jointTraj)):
        D = jointTraj[len(jointTraj)-1-i][1]()
        envLabels = {}
        for env in D.keys():
            colourReward = env[0]()
            label = ""
            for colour in orderOfTrapColours:
                if(colourReward[colour]<0):
                    ...
                    label = label + "o"
            envLabels[env] = label
        D = {envLabels[env]:prob for env, prob in D.items()}    
        ...
        if(i==(len(jointTraj) -1)):
            ax[i].set(xlabel='Reward Function', ylabel="Step " + str(len(jointTraj) - i-1))
        else:
            ax[i].set(ylabel="Step " + str(len(jointTraj) - i-1))
